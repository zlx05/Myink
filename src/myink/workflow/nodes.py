...
from myink.memory.embedder import get_embedder
...
from myink.schemas import (AuditVerdict, ChapterCast, ChapterPlan, Finding,
                           MutationCandidate, ValidationReport)
from myink.validation.ledger_l2 import run_ledger_l2
from myink.validation.continuity import check_transition_anchor, repair_generated_transition_anchor
...
def _bounded_generate(chain: FallbackChain, messages: list[dict], *,
    ...
    from myink.providers.base import MODEL_REGISTRY
...
def node_validate(state: ChapterState) -> ChapterState:
    ...
    with tenant_session(pid) as db:
        ...
        from myink.workflow.review import rejected_proposal
...
def _load_outline_slice(db: Session, project_id: uuid.UUID, chapter_seq: int) -> dict | None:
    ...
    from myink.workflow.outline import covering_item
